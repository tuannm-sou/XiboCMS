...
{
    public static $DESC_5_STAR_MEDIA_LAYOUT = '5StarMedia Layout';
    public static $STATUS_5_STAR_MEDIA_DEFAULT_LAYOUT = 4;
    ...
    public function Edit($layoutId, $layout, $description, $tags, $userid, $retired)
    {
        ...
        try {
            ...
            $sth = $dbh->prepare('SELECT description FROM layout WHERE layoutID = :layoutid');
            $sth->execute(array(
                'layoutid' => $layoutId
            ));
            if (!$row = $sth->fetch())
                throw new Exception('No layout record found with that ID');

            $currentDesc = $row['description'];
            if (Layout::$DESC_5_STAR_MEDIA_LAYOUT == $currentDesc) {
                $SQL = 'UPDATE layout SET layout = :layout, modifiedDT = :modifieddt, retired = :retired WHERE layoutID = :layoutid';
                $sth = $dbh->prepare($SQL);
                $sth->execute(array(
                    'layout' => $layout,
    ...
    function fn_Import($uploadedFile, $layout, $userId, $template)
    {
        ...
        try {
            ...
            // Add the layout
            // [Tuan Nguyen] Use new behavior for adding layout
            if (!$layoutId = $this->fn_Add($layout, $userId, $xml))
                return false;
    ...
    function fn_Add($layout, $userid, $xml)
    {
    ...
    /**
     * Set 55StarMedia default layout.
     *
     * @param [type] $layoutId [description]
     */
    function SetDefaultLayout($layoutId)
    {
        try {
            ...
            // Update the layout record with the new status
            $SQL = '
                UPDATE layout
                    SET status = :status
             WHERE layoutID = :layoutID';
            ...
            // Reset status of last layout used by default to keep only one default layout
            $SQL = '
                UPDATE layout
                    SET status = 3
             WHERE layoutID != :layoutID
            ...
            if (!$this->IsError())
                return $this->SetError(__("Unable to set 5StarMedia default layout"));
