...
HTML;
    ...
    function LayoutGrid()
    {
        ...
        foreach ($layouts as $layout) {
            ...
            switch ($layout['status']) {
                ...
                case Layout::$STATUS_5_STAR_MEDIA_DEFAULT_LAYOUT:
                    $row['status'] = '<span title="' . __('5StarMedia default layout') . '" class="glyphicon glyphicon-star"></span>';
                    break;
            ...
            if (Layout::$DESC_5_STAR_MEDIA_LAYOUT == $row['description']) {
                $row['buttons'][] = array(
                    ...
                    'linkType' => '_self',
                    'url' => 'index.php?p=layout&q=SetDefaultLayout&layoutid=' . $layout['layoutid'],
                    'text' => __('Set default')
                );
    ...
    public function LayoutStatus()
    {
        ...
        switch ($layout->IsValid($layoutId)) {
            ...
            case Layout::$STATUS_5_STAR_MEDIA_DEFAULT_LAYOUT:
                $status = '<span title="' . __('5StarMedia default layout') . '" class="glyphicon glyphicon-star"></span>';
                break;
    ...
    public function ImportForm()
    {
        ...
        // [Tuan Nguyen] new option for 5StarMedia customized layout
        // Default value: true (checked)
        $formFields[] = FormManager::AddCheckbox('customizedLayout', __('5StarMedia customized layout?'),
            true,
            __('Use 5StartMedia customized layout. The validation will be ignored.'),
            '');
    ...
    public function Import()
    {
        ...
        $customizedLayout = Kit::GetParam('customizedLayout', _POST, _CHECKBOX);
        ...
        // [Tuan Nguyen] If the given layout isn't customized, use default behavior
        if (!$customizedLayout) {
            if (!$layoutObject->Import($fileLocation, $layout, $this->user->userid, $template, $replaceExisting, $importTags)) {
                trigger_error($layoutObject->GetErrorMessage(), E_USER_ERROR);
            }
        } else {
            // [Tuan Nguyen] Use new behavior for importing layout
            if (!$layoutObject->fn_Import($fileLocation, $layout, $this->user->userid, $template)) {
                trigger_error($layoutObject->GetErrorMessage(), E_USER_ERROR);
            }
        }
    ...
    function SetDefaultLayout()
    {
        ...
        if (!$layout->SetDefaultLayout($layoutId)) {
            trigger_error($layout->GetErrorMessage(), E_USER_ERROR);
        }
